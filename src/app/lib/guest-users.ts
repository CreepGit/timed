...
import type { TGuestResponse } from "../../pocketbase-types.ts"
...
export async function getGuestUser(c: Context): Promise<TGuestResponse | undefined> {
    const userId = cookie.getCookie(c, GUEST_USER_COOKIE)
    if (!userId) return undefined
    const user = await utils.pb.get(pb.collection("tGuest").getOne(userId))
    if (!user) {
...
export async function getOrCreateGuestUser(c: Context): Promise<{ created: boolean, user: TGuestResponse }> {
    const existing = await getGuestUser(c)
    if (existing) return { created: false, user: existing }

    const user = await pb.collection("tGuest").create({})
